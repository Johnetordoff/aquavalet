...
class TestDelete:
    ...
    async def test_delete(self, http_server_client):
        ...
        async with MockOsfstorageServer() as server:
            ...
            response = await http_server_client.fetch(path, method='DELETE', allow_nonstandard_methods=True)
...
class TestDownload:
    ...
    async def test_download_zip(self, http_server_client):
        file_metadata = get_file_metadata_json()
        folder_metadata = get_folder_metadata_json()
        path = make_path('osfstorage', 'osfstorage', 'guid0', folder_metadata["data"]["id"], serve='download_as_zip')
        ...
        async with MockOsfstorageServer() as server:
            ...
            resp = await http_server_client.fetch(path, method='GET', allow_nonstandard_methods=True)
...
class TestIntraCopy:
    ...
    async def test_intra_copy(self, http_server_client):
        file_metadata = get_file_metadata_json()
        path = make_path('osfstorage', 'osfstorage', 'guid0', file_metadata["data"]["id"], to='/osfstorage/guid0/', destination_provider='osfstorage')
        ...
        async with MockOsfstorageServer() as server:
            ...
            response = await http_server_client.fetch(path, method='COPY', allow_nonstandard_methods=True)
...
class TestUpload:
    ...
    async def test_upload(self, http_server_client):
        ...
        async with MockOsfstorageServer() as server:
            ...
            response = await http_server_client.fetch(path, method='UPLOAD', allow_nonstandard_methods=True, body=b'12345')
    ...
    async def test_upload_warn(self, http_server_client):
        folder_metadata = get_folder_metadata_json()
        path = make_path('osfstorage', 'osfstorage', 'guid0', folder_metadata["data"]["id"], new_name='test', conflict='warn')

        async with MockOsfstorageServer() as server:
            server.mock_metadata(folder_metadata)
            server.mock_409(folder_metadata)
            resp = await http_server_client.fetch(path, method='UPLOAD', allow_nonstandard_methods=True, raise_error=False)
    ...
    async def test_upload_new_version(self, http_server_client):
        folder_metadata = get_folder_metadata_json()
        path = make_path('osfstorage', 'osfstorage', 'guid0', folder_metadata["data"]["id"], new_name='test', conflict='version')
...
class TestRename:
    ...
    async def test_rename(self, http_server_client):
        file_metadata = get_file_metadata_json()
        path = make_path('osfstorage', 'osfstorage', 'guid0', file_metadata["data"]["id"], new_name='new_name')

        async with MockOsfstorageServer() as server:
            server.mock_metadata(file_metadata)
            server.mock_rename(file_metadata)

            response = await http_server_client.fetch(path, method='RENAME', allow_nonstandard_methods=True)
...
class TestCreateFolder:
    ...
    async def test_create_folder(self, http_server_client):
        ...
        path = make_path('osfstorage', 'osfstorage', 'guid0', folder_metadata["data"]["id"], new_name='new_test_folder')

        async with MockOsfstorageServer() as server:
            server.mock_metadata(folder_metadata)
            server.mock_create_folder(folder_metadata)

            response = await http_server_client.fetch(path, method='CREATE_FOLDER', allow_nonstandard_methods=True)
