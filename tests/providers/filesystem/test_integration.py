...
class TestIntraCopy:
    ...
    async def test_copy(self, http_server_client, fs):
        ...
        url = (
            urllib.parse.quote("/filesystem/test.txt")
            + "?to=test_folder/&destination_provider=filesystem"
        )
        response = await http_server_client.fetch(
            url, method="COPY", allow_nonstandard_methods=True
        )
        assert response.code == 200
...
class TestUpload:
    ...
    async def test_upload(self, http_server_client, fs):
        ...
        url = (
            urllib.parse.quote("/filesystem/")
            + "?destination_provider=filesystem&new_name=test.txt"
        )
        response = await http_server_client.fetch(
            url, method="UPLOAD", allow_nonstandard_methods=True, body=b"test"
        )
    ...
    async def test_upload_warn(self, http_server_client, fs):
        ...
        resp = await http_server_client.fetch(
            url, method="UPLOAD", allow_nonstandard_methods=True, raise_error=False
        )
    ...
    async def test_upload_replace(self, http_server_client, fs):
        ...
        url = (
            urllib.parse.quote("/filesystem/")
            + "?destination_provider=filesystem&new_name=test.txt&conflict=replace"
        )
        response = await http_server_client.fetch(
            url, method="UPLOAD", body=b"new file", allow_nonstandard_methods=True
        )
        assert response.code == 201
    ...
    async def test_upload_rename(self, http_server_client, fs):
        ...
        url = (
            urllib.parse.quote("/filesystem/")
            + "?destination_provider=filesystem&new_name=test.txt&conflict=rename"
        )
        response = await http_server_client.fetch(
            url, method="UPLOAD", allow_nonstandard_methods=True
        )
        assert response.code == 201
